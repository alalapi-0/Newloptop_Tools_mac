...
load_profile() {
  ...
  if [[ -z "${MEDIA_LIST:-}" ]]; then
    log_err "MEDIA_LIST is not set in profile: ${profile_file}"
    exit 1
  fi
}
...
if ! command -v brew >/dev/null 2>&1; then
  log_err "Homebrew is required for media packages. Run --only homebrew first."
  exit 1
fi

load_profile "${SETUP_PROFILE}"

media_list_path="${MEDIA_LIST}"
if [[ "${media_list_path}" != /* ]]; then
  media_list_path="${SETUP_REPO_ROOT}/${media_list_path}"
fi

if [[ ! -f "${media_list_path}" ]]; then
  log_err "Media list not found: ${media_list_path}"
  exit 1
fi

log_info "Using media list: ${media_list_path}"
...
while IFS= read -r line || [[ -n "${line}" ]]; do
  ...
  if [[ "${SETUP_DRY_RUN}" -eq 1 ]]; then
    log_info "[dry-run] Would install formula: ${line}"
    continue
  fi
